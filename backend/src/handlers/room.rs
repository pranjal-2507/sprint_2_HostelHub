...
pub async fn get_all_rooms(
    ...
    let rooms = sqlx::query_as::<Postgres, Room>(
        "SELECT id, hostel_id, room_number, floor, capacity, occupancy, room_type, status, price_per_month::FLOAT8, created_at FROM rooms ORDER BY room_number"
    )
    .fetch_all(&state.db)
    .await
...
pub async fn get_rooms(
    ...
    let rooms = sqlx::query_as::<Postgres, Room>(
        "SELECT id, hostel_id, room_number, floor, capacity, occupancy, room_type, status, price_per_month::FLOAT8, created_at FROM rooms WHERE status = 'available' ORDER BY room_number"
    )
    .fetch_all(&state.db)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_room_by_id(
    ...
    let room = sqlx::query_as::<Postgres, Room>(
        "SELECT id, hostel_id, room_number, floor, capacity, occupancy, room_type, status, price_per_month::FLOAT8, created_at FROM rooms WHERE id = $1"
    )
    .bind(id)
    .fetch_one(&state.db)
    .await
